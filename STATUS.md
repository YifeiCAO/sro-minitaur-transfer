...
| **across-domain matrix** | identification top-1 on 8 cross-domain pairs | ✅ **within ≫ across:** across mean id_top1 = **0.107 ≈ chance 0.10** (all 8 pairs null) vs within ~0.20 → transfer is **domain-specific**, not generic conditioning. _Caveat: across pairs have different response vocab, partly confounding domain with format_ |
| **base-rate control (9 pairs)** | own-A vs marginal-matched stranger; permutation p | ✅ **7/9 pairs survive base-rate** (real person-specific signal beyond choice rate). Cleanest: **CCT cold↔hot −0.02 (p=5e-5)**, conflict DF/DPX↔recent −0.002…−0.003 (p<.05). Discounting weak/inconsistent (1 null, 1 borderline). _Suggestive saturation gradient (balanced targets → larger residual, r=−0.86 overall) but **CCT-driven**; among genuinely-different-task pairs rho=−0.58, p=0.17 (n=7, underpowered) → a lead, not established_ |
| **retest ceiling** | same-task time1→time2 identification (upper bound) | pending — optional magnitude anchor |
